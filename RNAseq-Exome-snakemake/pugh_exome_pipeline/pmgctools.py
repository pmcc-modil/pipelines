...
def read_vars(ini=None):
    ...
    tools = re.split('\d+\)', out)[1:]
    global VARS
    VARS = {}
    module_pattern = re.compile(r'\s*(.+)/(\S+)')
    for i in tools:
        matched = module_pattern.match(i.strip().split()[0])
        if matched:
            if matched.group(1) in VARS:
                print("Error: multiple {} modules were loaded. I am confused.".format(matched.group(1)), file=sys.stderr)
                exit(1)
            else:
                VARS[matched.group(1)] = matched.group(2)
